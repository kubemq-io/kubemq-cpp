    ...
    try {
        ...
        // Build final report
        {
            ...
            last_report_ = {
                ...
                {"stopped_at", NowIso8601()},
                {"duration_seconds", metrics_.ElapsedSeconds()},
                {"metrics", metrics_.Snapshot()},
...
nlohmann::json Engine::GetRunStatus() const {
    ...
    if (snapshot.contains("_totals")) {
        totals = snapshot["_totals"];
    }
    // Add lost and duplicated (not in _totals by default)
    uint64_t total_lost = 0, total_duplicated = 0;
    ...
    totals["lost"] = total_lost;
    totals["duplicated"] = total_duplicated;

    // Build pattern_states
    std::string current_state = State();
    nlohmann::json pattern_states;
    for (const auto& [name, data] : snapshot.items()) {
        if (name == "_totals" || !data.is_object()) continue;
        pattern_states[name] = (current_state == kStateRunning) ? "running" : current_state;
    }
