...
// EventsStoreWorker sends and receives persistent events on a single channel.
class EventsStoreWorker : public Worker {
public:
    EventsStoreWorker(const std::string& broker_address,
                      const std::string& client_id_prefix,
                      const std::string& channel,
                      int rate,
                      PatternMetrics& metrics);
    ~EventsStoreWorker() override;

    void Start() override;
    void Stop() override;
    std::string PatternName() const override { return "events_store"; }
