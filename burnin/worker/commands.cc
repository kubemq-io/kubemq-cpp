...
CommandsWorker::CommandsWorker(const std::string& broker_address,
...
CommandsWorker::~CommandsWorker() {
    Stop();
}

void CommandsWorker::Start() {
    running_.store(true);
    responder_thread_ = std::thread(&CommandsWorker::ResponderLoop, this);
    // Small delay to let the responder subscribe before sending
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    sender_thread_ = std::thread(&CommandsWorker::SenderLoop, this);
}

void CommandsWorker::Stop() {
...
void CommandsWorker::SenderLoop() {
    kubemq::ClientOptions opts;
    opts.set_address(broker_address_);
    opts.set_client_id(client_id_prefix_ + "-cmd-sender");

    auto client_or = kubemq::Client::Create(opts);
    if (!client_or.ok()) {
        std::cerr << "[CommandsWorker] Sender connect failed: "
    ...
    while (running_.load()) {
        ++seq;
        std::string body = "command-" + std::to_string(seq);

        auto resp_or = sender_client_->SendCommandSimple(
...
void CommandsWorker::ResponderLoop() {
    kubemq::ClientOptions opts;
    opts.set_address(broker_address_);
    opts.set_client_id(client_id_prefix_ + "-cmd-responder");

    auto client_or = kubemq::Client::Create(opts);
    if (!client_or.ok()) {
        std::cerr << "[CommandsWorker] Responder connect failed: "
                  << client_or.status().ToString() << "\n";
        metrics_.errors.fetch_add(1, std::memory_order_relaxed);
        return;
    }
    ...
    auto sub_or = responder_client_->SubscribeToCommands(
        channel_, "",
        [this](const kubemq::CommandReceive& cmd) {
            // Build and send response
            auto reply_or = kubemq::CommandReply::Builder()
                .SetRequestId(cmd.id)
                .SetResponseTo(cmd.response_to)
                .SetClientId(client_id_prefix_ + "-cmd-responder")
                .Build();

            if (reply_or.ok()) {
                auto status = responder_client_->SendCommandResponse(*reply_or);
                if (!status.ok()) {
                    metrics_.errors.fetch_add(1, std::memory_order_relaxed);
                }
            }
    ...
    if (!sub_or.ok()) {
        std::cerr << "[CommandsWorker] Subscribe failed: "
