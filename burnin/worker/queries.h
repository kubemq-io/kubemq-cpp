...
// QueriesWorker sends queries and responds on a single channel.
class QueriesWorker : public Worker {
public:
    QueriesWorker(const std::string& broker_address,
                  const std::string& client_id_prefix,
                  const std::string& channel,
                  int rate,
                  PatternMetrics& metrics);
    ~QueriesWorker() override;

    void Start() override;
    void Stop() override;
    std::string PatternName() const override { return "queries"; }
