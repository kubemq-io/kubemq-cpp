...
EventStreamHandle::EventStreamHandle() : impl_(std::make_unique<Impl>()) {}

EventStreamHandle::~EventStreamHandle() {
    Close();
}

Status EventStreamHandle::Send(const Event& event) {
    ...
    if (impl_->done.load(std::memory_order_acquire)) {
        return Status(ErrorCode::kFatal, "event stream is closed", "EventStreamHandle::Send",
                      event.channel(), "");
    }
    if (!impl_->stream->Write(proto_event)) {
        return Status(ErrorCode::kTransient, "failed to write to event stream",
                      "EventStreamHandle::Send", event.channel(), "");
    }
    return Status();
}

void EventStreamHandle::Close() {
    {
        std::lock_guard<std::mutex> lock(impl_->write_mu);
...
bool EventStreamHandle::IsDone() const {
    return impl_->done.load(std::memory_order_acquire);
}

void EventStreamHandle::Wait() {
    impl_->JoinReader();
}

void EventStreamHandle::SetOnError(std::function<void(const Status&)> callback) {
    std::lock_guard<std::mutex> lock(impl_->callback_mu);
    impl_->on_error = std::move(callback);
}

Subscription::Subscription() : impl_(std::make_unique<Impl>()) {}
...
bool Subscription::IsDone() const {
    return impl_->done.load(std::memory_order_acquire);
}
...
Status Client::SendEvent(const Event& event) {
    ...
    // 4. Validate
    s = internal::ValidateChannelNoWildcard(channel, "SendEvent");
    if (!s.ok())
        return s;

    s = internal::ValidateBody(event.body(), event.metadata(), "SendEvent");
    if (!s.ok())
        return s;

    // 5. Convert to proto Event (Store=false)
    ::kubemq_pb::Event proto_event;
    proto_event.set_eventid(event.id().empty() ? internal::GenerateUUID() : event.id());
    ...
    // 6. Call transport.SendEvent() (idempotent=true, retry handled by transport)
    return impl_->transport->SendEvent(proto_event);
}

StatusOr<std::unique_ptr<EventStreamHandle>> Client::SendEventStream(
    ...
    // 3. Create EventStreamHandle wrapping the stream
    auto handle = std::unique_ptr<EventStreamHandle>(new EventStreamHandle());
    handle->impl_->ctx = std::move(ctx);
    handle->impl_->stream = std::move(*stream_or);
    handle->impl_->opts = impl_->opts;

    // 4. Store callback before starting reader (no race)
...
StatusOr<std::unique_ptr<EventStreamHandle>> Client::SendEventStream() {
    return SendEventStream(nullptr);
}

StatusOr<std::unique_ptr<Subscription>> Client::SubscribeToEvents(
    ...
    // 3. Validate channel (wildcards allowed for subscriptions)
    s = internal::ValidateChannel(effective_channel, "SubscribeToEvents");
    if (!s.ok())
        return s;

    // 4. Build Subscribe proto
    ::kubemq_pb::Subscribe sub;
    sub.set_subscribetypedata(::kubemq_pb::Subscribe::Events);
    sub.set_clientid(impl_->opts.client_id());
    sub.set_channel(effective_channel);
    sub.set_group(group);

    // 5. Call transport SubscribeToEvents (server stream)
            ...
            while (!sub_impl->done.load(std::memory_order_acquire)) {
                ...
                while (reader->Read(&proto_event)) {
                    ...
                    internal::SafeInvoke(
                        ...
                        on_error, "SubscribeToEvents::OnEvent");
                }
                    ...
                    [&]() {
                        if (on_error) {
                            on_error(Status(
                                ...
                                "event subscription stream ended: " + last_status.error_message(),
                                "SubscribeToEvents", "", ""));
                        }
                    },
                    nullptr, "SubscribeToEvents::OnError");
            }
        });
...
Status Client::PublishEvent(const std::string& channel, const std::string& body,
